...
--[[
    areas: Protector Blocks for Area Protection Mod
...
local function del_display(pos)
	...
	for _, v in ipairs(objects) do
		...
		if v and v:get_luaentity() and v:get_luaentity().name == "areas:display" then
			v:remove()
		end
	end
end
...
local common_def = {
	on_place = function(itemstack, placer, pointed_thing)
		...
		if in_delay[pname] then
			minetest.chat_send_player(pname,
				S("You cannot place more than 1 protector blocks within @1 seconds!", areas.config.protector_delay))
			return itemstack
		end
		...
		local pos1 = vector.add(pos, { x = areas.config.protector_radius, y = areas.config.protector_radius, z = areas.config.protector_radius })
		local pos2 = vector.add(pos, { x = areas.config.protector_radius * -1, y = areas.config.protector_radius * -1, z = areas.config.protector_radius * -1 })
		local canAdd, errMsg = areas:canPlayerAddArea(pos1, pos2, pname)
		if not canAdd then
			minetest.chat_send_player(pname, S("You can't protect that area: @1", errMsg))
			return itemstack
		end
		...
		minetest.chat_send_player(pname, S("Area protected. ID: @1", id))
		...
		in_delay[pname] = true
		minetest.after(areas.config.protector_delay, function()
	...
	on_dig = function(pos, node, digger)
		...
		if not areas:isAreaOwner(id, name) then
			minetest.chat_send_player(name, S("Area @1 does not exist or is not owned by you.", id))
			return false
		end
		if minetest.node_dig(pos, node, digger) then
			areas:remove(id)
			areas:save()
			minetest.chat_send_player(name, S("Removed area @1", id))
			return true
		else
	...
	on_punch = function(pos, node, puncher)
		...
		if not areas.areas[id] then
			minetest.chat_send_player(name, S("The area @1 does not exist.", id))
		...
		minetest.chat_send_player(name, S("Area @1 selected.", id))
		minetest.add_entity(pos, "areas:display")
		if minetest.get_modpath("vizlib") then
			vizlib.draw_area(vector.add(pos1, -0.5), vector.add(pos2, 0.5), {
				color = "#7ba428",
...
minetest.register_node("areas:protect", override({
...
-- TODO: Fix rotation
-- minetest.register_node("areas:protect_logo", override({
...
minetest.register_entity("areas:display", {
	...
	on_step = function(self, dtime)
		...
		if self.timer > areas.config.protector_show then self.object:remove() end
	end
})

local r = areas.config.protector_radius

local r = (areas.config.protector_radius or 5)
...
core.register_alias("protect_block_area:protect", "areas:protect")
